...
public class Money {
    ...
    public Money sum(Money other) {
        return new Money(mainPart + other.mainPart, subPart + other.subPart);
    }
    ...
    public Money increase(int percent)
    {
        // посчитаем, сколько надо добавить
//        int toIncrease = (int)Math.round(absolute() / 100.0) * percent;
        int toIncrease = (int)Math.round(absolute() / 100.0 * percent);
//        int tmp = absolute();
//        int res = (int)((tmp * percent)/100.0);
//        this.subPart = this.subPart + res;
//        normalize();
//        return new Money(mainPart, subPart );
        return new Money(mainPart, subPart +toIncrease);
    }

    public Money IncreaseWithRoundingToSmaller(int percent)
    {
        int toIncrease = (int)Math.floor(absolute() / 100.0 * percent);
        return new Money(mainPart, subPart +toIncrease);
    }

    public Money IncreaseWithRoundingToBigger(int percent)
    {
        int toIncrease = (int)Math.ceil(absolute() / 100.0 * percent);
        return new Money(mainPart, subPart +toIncrease);
    }
